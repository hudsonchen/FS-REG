...
class loss_classification_list:
    ...
    def llk_classification(self,
        ...
        log_y_hat = jax.nn.log_softmax(self.apply_fn(params, state, rng_key, x)[0], axis=1)
        log_likelihood = jnp.mean(jnp.sum(log_y_hat * y, axis=1), axis=0)
        ...
        if self.element_wise:
            ...
            for i in range(self.class_num):
                ...
                if self.inverse:
                    ...
                    freg += jnp.squeeze(y_ntk_.T @ jnp.linalg.inv(ntk_ + jnp.eye(self.dummy_input_dim)) @ y_ntk_)
                else:
